...
def simulation_test():
    ...
    for index in range(number):
        ...
        plt.axis("off")
        plt.imshow(im.permute(1,2,0) * 0.5 + 0.5)
        plt.savefig("/home/abaozheng6/View_Synthesis/synsin/demos/image_test_0217_result/input/test_in_{}.png".format(str(index).zfill(2)), bbox_inches = 0)
        plt.axis("off")
        plt.imshow(pred_imgs[0].squeeze().cpu().permute(1,2,0).numpy() * 0.5 + 0.5)
        plt.savefig("/home/abaozheng6/View_Synthesis/synsin/demos/image_test_0217_result/pred/test_pred_{}.png".format(str(index).zfill(2)), bbox_inches = 0)
